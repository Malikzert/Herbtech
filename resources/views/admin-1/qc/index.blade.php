    ...
    <div class="mb-6">
        <div class="bg-glass rounded-xl border border-white/50 p-4 shadow-sm">
            <form method="GET" action="{{ route('admin.qc.index') }}" class="flex flex-wrap gap-3 items-center">
                ...
                <select name="action" class="modern-select h-11 px-4 py-2 bg-white/50 border border-gray-200 rounded-lg text-sm text-gray-700 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 focus:outline-none focus:bg-white transition cursor-pointer">
                ...
                <a href="{{ route('admin.qc.index') }}" class="h-11 px-5 bg-gray-100 text-gray-700 font-medium rounded-lg hover:bg-gray-200 transition flex items-center gap-2">
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead class="bg-gray-50/80 text-gray-500 text-xs uppercase">
                    <tr>
                        <th class="px-6 py-3.5 font-medium text-left">ID QC</th>
                        <th class="px-6 py-3.5 font-medium text-left">Batch</th>
                        <th class="px-6 py-3.5 font-medium text-left">Inspector</th>
                        <th class="px-6 py-3.5 font-medium text-left">Inspeksi</th>
                        <th class="px-6 py-3.5 font-medium text-center">Passed</th>
                        <th class="px-6 py-3.5 font-medium text-center">Rejected</th>
                        <th class="px-6 py-3.5 font-medium text-left">Hasil</th>
                        <th class="px-6 py-3.5 font-medium text-left">Action</th>
                ...
                <tbody class="divide-y divide-gray-100/50">
                    ...
                    <tr class="hover:bg-gray-50/50 transition">
                        ...
                        <td class="px-6 py-4 text-sm text-gray-600">{{ $qc->inspector_name }}</td>
                        ...
                        <td class="px-6 py-4">
                            @switch($qc->status)
                                @case('passed')
                                    <span class="px-2.5 py-1 text-xs font-medium rounded-full bg-emerald-100/60 text-emerald-700">Passed</span>
                                    @break
                                @case('partial_reject')
                                    <span class="px-2.5 py-1 text-xs font-medium rounded-full bg-amber-100/60 text-amber-700">Partial</span>
                                    @break
                                @case('full_reject')
                                    <span class="px-2.5 py-1 text-xs font-medium rounded-full bg-red-100/60 text-red-700">Full Reject</span>
                                    @break
                            @endswitch
                        </td>
                        <td class="px-6 py-4">
                            @switch($qc->action)
                                @case('release')
                                    <span class="px-2.5 py-1 text-xs font-medium rounded-full bg-blue-100/60 text-blue-700">Release</span>
                                    @break
                                @case('rework')
                                    <span class="px-2.5 py-1 text-xs font-medium rounded-full bg-purple-100/60 text-purple-700">Rework</span>
                                    @break
                                @case('reject')
                                    <span class="px-2.5 py-1 text-xs font-medium rounded-full bg-gray-100/60 text-gray-600">Reject</span>
                                    @break
                            @endswitch
                        </td>
                        <td class="px-6 py-4 text-right">
                            <a href="{{ route('admin.qc.show', $qc->id) }}" class="text-emerald-600 hover:text-emerald-700 text-sm font-medium">Detail</a>
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="9" class="px-6 py-16 text-center">
                            <div class="flex flex-col items-center">
                                ...
                                <p class="text-gray-500 font-medium">Belum ada data QC</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
