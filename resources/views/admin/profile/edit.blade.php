...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2 space-y-6">
        ...
        <div class="bg-glass rounded-xl border border-white/50 p-6 shadow-sm">
            ...
            @if(session('success'))
            <div class="mb-6 p-4 bg-emerald-500/30 text-gray-800 rounded-lg border border-emerald-400/50">
                {{ session('success') }}
            </div>
            @endif
            ...
            <form action="{{ route('admin.profile.update') }}" method="POST" class="space-y-4">
                ...
                <div class="pt-2">
                    <button type="submit" class="px-6 py-2.5 btn-glass text-gray-800 text-sm font-medium rounded-lg hover:bg-emerald-700 transition">
                        Simpan Perubahan
                    </button>
                </div>
            </form>
        ...
        <div class="bg-glass rounded-xl border border-white/50 p-6 shadow-sm">
            ...
            <form action="{{ route('admin.profile.password') }}" method="POST" class="space-y-4">
                ...
                <div>
                    <label class="block text-sm font-bold text-gray-800 mb-1">Password Lama</label>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-bold text-gray-800 mb-1">Password Baru</label>
    ...
    <div class="space-y-6">
        <div class="bg-glass rounded-xl border border-white/50 p-6 text-center shadow-sm">
            ...
            <span class="inline-block mt-2 px-3 py-1 text-xs font-bold rounded-full bg-emerald-500/40 text-gray-800 border border-emerald-400/50">
                {{ ucfirst($user->role) }}
            </span>
        </div>
