...
<div class="space-y-6 max-w-2xl">
    <div class="flex justify-between items-center">
        <a href="{{ route('operator.qc.show', $qc->id) }}" class="text-blue-800 hover:text-blue-900 font-medium flex items-center gap-2">
    ...
    <div class="bg-white/60 backdrop-blur-md rounded-xl border border-white/20 p-6 shadow-sm glass-card">
        <form action="{{ route('operator.qc.update', $qc->id) }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <select name="production_id" required class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-700">
            ...
            <div>
                ...
                <input type="number" name="total_inspected" value="{{ old('total_inspected', $qc->total_inspected) }}" required min="1" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-700">
                @error('total_inspected') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
            </div>
            
            <div class="grid grid-cols-2 gap-6">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1 text-blue-800">Total Passed</label>
                    <input type="number" name="total_passed" value="{{ old('total_passed', $qc->total_passed) }}" required min="0" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-700 text-blue-900 font-bold">
            ...
            <div class="flex justify-end gap-3 pt-4 border-t border-gray-100">
                <button type="submit" class="px-6 py-2.5 bg-blue-800 text-white rounded-lg hover:bg-blue-900 font-medium shadow-md">Simpan Perubahan QC</button>
            </div>
        </form>
    </div>
</div>
